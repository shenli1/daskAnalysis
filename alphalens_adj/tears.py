...
def create_returns_tear_sheet(
    ...
    #)
    binReturn,binStd = perf.mean_return_by_quantile(factor_data,by_date=True,demeaned=False)
    ...
    for x in cum_ret.columns:
        ...
        if tmp > maxCumReturn:
            maxCumCol = x
            maxCumReturn = tmp
    factor_returns = ret_wide[[maxCumCol]]
    factor_returns = factor_returns.rename(columns={maxCumCol:'1D'})
